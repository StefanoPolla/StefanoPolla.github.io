---
layout: page
title: openings
permalink: /openings/
nav: true
nav_order: 9
---

### [PhD in Quantum Algorithms for Chemistry](https://werkenbij.uva.nl/en/vacancies/phd-in-quantum-algorithms-for-chemistry-netherlands-15197)

**Deadline:** 20 July 2026

Are you excited about designing the next generation of quantum algorithms and turning them into tools for molecular science? Quantum computing holds the promise to transform the toolset we use to understand molecules and materials, and this project focuses on developing, analyzing, and optimizing those algorithms before large-scale quantum devices become available.

[View full opening and apply](https://werkenbij.uva.nl/en/vacancies/phd-in-quantum-algorithms-for-chemistry-netherlands-15197)

---

No other PhD or Postdocs positions are currently open in our group. More might come soon...

